...
template <typename T>
concept isTreasure = same_as<T, Treasure<int, false>> || same_as<T, Treasure<int, true>>;
...
concept isAdventurer = requires(T t)
{
  ...
  // t.loot((Treasure<int, true>){t.pay()});
  t.loot((Treasure<int, false>){t.pay()});
};
...
// run()
template <isAdventurer A, isAdventurer B>
...
template <isAdventurer A, isTreasure B>
constexpr void run(Encounter<A, B> encounter)
{
  encounter.first.loot(encounter.second.getLoot());
}

template <isTreasure A, isAdventurer B>
constexpr void run(Encounter<A, B> encounter)
{
...
constexpr void run(Encounter<A, B> encounter)
{
  ...
  std::cout << requires { isTreasure<A>; }
  << " " << requires { isTreasure<B>; }
  << std::endl;
  // 2 Treasurees
  (void)encounter;
